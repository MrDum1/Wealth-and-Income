...
plot2 <-income_mean %>%
  ...
  scale_y_continuous(
    limits = c(0, 60000),
    breaks = function(x) seq(0, 60000, length.out = 6)  # 5 segments create 6 breaks
  ) +
  labs(
    x = "Year",
    y = "Average Income (Inflation-Adjusted to 2019 Dollars)",
...
```{r}
income_mean %>%
 filter(!(race %in% c("Asian Alone or in Combination", "Black Alone or in Combination", 
"White, Not Hispanic"))) %>%
 filter(dollar_type %in% "2019 Dollars") %>%
 filter(income_quintile %in% "Middle") %>%
 ggplot() +
  aes(x = year, y = income_dollars, colour = race) +
  geom_jitter() +
   geom_smooth(method = "gam", formula = y ~ s(x), se = FALSE) + 
   scale_x_continuous(limits = c(1967, 2019), breaks = seq(1967, 2019, length.out = 14)  # 10 intervals will create 11 breaks
  ) +
  labs(
    x = "Year",
    ...
    caption = "Figure 1: Trends in the lowest income quintile among different racial groups from 1967 to 2019. The plot illustrates how income levels have varied over time for each group, with a smoothed line showing general trends."
  ) +
  theme_minimal() +
...
"White, Not Hispanic"))) %>%
 filter(dollar_type %in% "2019 Dollars") %>%
 filter(income_quintile %in% "Fourth") %>%
 ggplot() +
  aes(x = year, y = income_dollars, colour = race) +
  geom_jitter() +
  geom_smooth(method = "gam", formula = y ~ s(x), se = FALSE) + 
...
"White, Not Hispanic"))) %>%
 filter(dollar_type %in% "2019 Dollars") %>%
 filter(income_quintile %in% "Highest") %>%
 ggplot() +
  aes(x = year, y = income_dollars, colour = race) +
  geom_jitter() +
   geom_smooth(method = "gam", formula = y ~ s(x), se = FALSE) + 
